...
impl StateMachine for WeirdSwitchMachine {
    ...
    fn next_state(starting_state: &TwoSwitches, t: &Toggle) -> TwoSwitches {
        match t {
            Toggle::FirstSwitch => {
                let new_first_switch_state = !starting_state.first_switch;

                let new_second_switch_state = if new_first_switch_state {
                    starting_state.second_switch
                } else {
                    false
                };
                TwoSwitches {
                    first_switch: new_first_switch_state,
                    second_switch: new_second_switch_state,
                }
            }
            Toggle::SecondSwitch => {
                let new_second_switch_state = !starting_state.second_switch;
