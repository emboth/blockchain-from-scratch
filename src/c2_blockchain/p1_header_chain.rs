...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        let mut current_parent_hash = hash(&self);

        for header in chain {
            if header.parent != current_parent_hash {
                return false;
            }
            current_parent_hash = hash(&header);
        }
...
fn build_valid_chain_length_5() -> Vec<Header> {
    ...
    for _ in 0..4 {
        let child_header = chain.last().unwrap().child();
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    let mut invalid_header = chain.last().unwrap().child();
    invalid_header.parent = 999999; // Deliberately incorrect parent hash
